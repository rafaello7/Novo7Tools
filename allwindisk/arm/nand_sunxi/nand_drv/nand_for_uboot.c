...
/* if partname is not null, partition with this name is erased;
 * otherwise the specified partition number is erased
 */
int nand_erase_uboot(const char *partname, int dev_num)
{
	int start;
	int size;
	char *tmp_buf;
	...
	{
		start =  sunxi_nand_getpart_offset_byname(partname);
		size  =  sunxi_nand_getpart_size_byname(partname);
	}
...
/*******************************************************************************
*º¯ÊýÃû³Æ: mark_bad_block
*º¯ÊýÔ­ÐÍ£ºmark_bad_block( uint chip_num, uint blk_num )
*º¯Êý¹¦ÄÜ: ±ê¼Çblk_numÖ¸¶¨µÄ¿éÎª»µ¿é¡£
*Èë¿Ú²ÎÊý: chip_num           chip number
           blk_num            ¿éºÅ
*·µ »Ø Öµ: 0              ±à³Ì²Ù×÷³É¹¦
*          other          ±à³Ì²Ù×÷Ê§°Ü
*±¸    ×¢:
...
/*******************************************************************************
*º¯ÊýÃû³Æ: NAND_BadBlockScan
*º¯ÊýÔ­ÐÍ£ºbad_blcok_scan(const boot_nand_para_t *nand_param)
*º¯Êý¹¦ÄÜ: ±ê¼Çblk_numÖ¸¶¨µÄ¿éÎª»µ¿é¡£
*Èë¿Ú²ÎÊý: nand_param
*·µ »Ø Öµ: >0              ±à³Ì²Ù×÷³É¹¦
*          -1              ±à³Ì²Ù×÷Ê§°Ü
