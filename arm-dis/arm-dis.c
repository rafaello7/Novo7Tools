		   ...
		   bfd_boolean little)
{
  ...
  for (;;)
      ...
      if (addr == 0)
	{
      ...
      addr -= 2;
      info->read_memory_func (addr, (bfd_byte *) b, 2, info);
  ...
  if (info->private_data == NULL)
    {
      ...
      private.features.core = -1UL;
      private.features.coproc = -1UL;
  ...
  if (!is_thumb)
    {
	 ...
	 is four bytes long and is either ordered 0123 or 3210.  */
      ...
      info->read_memory_func (pc, (bfd_byte *) b, 4, info);
      if (little_code)
  ...
  else
    {
	 ...
	 in the first two bytes.  */
      ...
      info->read_memory_func (pc, (bfd_byte *) b, 2, info);
      if (little_code)
	given = (b[0]) | (b[1] << 8);
      else
	given = (b[1]) | (b[0] << 8);
...
static unsigned addr_beg;
static unsigned char bytes[8192];

static void read_memory(bfd_vma memaddr, bfd_byte *myaddr, unsigned int length,
     struct disassemble_info *dinfo)
{
    memcpy(myaddr, bytes + memaddr - addr_beg, length);
}

static void my_fprintf(void *stream, const char *fmt, ...)
{
    va_list args;

    va_start(args, fmt);
    vsprintf(stream + strlen(stream), fmt, args);
    va_end(args);
}
...
int main(int argc, char *argv[])
{
    ...
    if( argc == 1 ) {
        printf("usage: arm-dis <file> [ <startaddr> ]\n");
        return 0;
    }
    memset(&info, 0, sizeof(struct disassemble_info));
    info.read_memory_func = read_memory;
    info.fprintf_func = my_fprintf;
    info.stream = stream_buf;
    ...
    addr_beg = 0x40000000;
    if( argc > 2 )
        addr_beg = strtoul(argv[2], NULL, 0);
    while( (rd = read(fd, bytes, sizeof(bytes))) > 0 ) {
        ...
        while( addr_offset < rd ) {
            ...
            for(i = 0; i < octets; ++i) {
                unsigned char c = bytes[addr_offset + i];
                if( c < 32 || c >= 127 )
                    c = '.';
                printf("%c", c);
            }
